...
We define some terminologies in this subsection that are later used in this
chapter. If the reader is already familiar with the requirements discussed in
above, they may consider skipping this subsection. 
\begin{itemize}
    \item A one sort set is a distinct collection of objects such
    that all the elements of the set are of single type (or sort). 
    \item  A term in logic represents an object in the domain of
    discourse.
    \item  A function $f: X \rightarrow Y$ is a mapping that associates each
    element of domain $(x \in X)$ with a unique element in co-domain $(y \in Y)$.
    \item  A function symbol represents an operation that maps elements of
    domain to unique element in the co-domain.
    \item The number of operands in a function (or operation) is the arity of
    the operation.
    \item A formula is finite sequence of symbols from the set of alphabets of a
    language.
    \item Term expressions is a composition of
...
\begin{itemize}
    ...
    such that $S$ is a set of one sort $s$, $F$ is a finite set of function symbols
    with their arities, and $E$ is a set of equations that are satisfied in $S$.

    \item A sub theory of a theory $(S,F,E)$ is defined as
    ...
    \item A \textit{signature} is a pair $\Sigma = (S,F)$ such that $S$ is set of
    sorts and $F$ is set of operation names.

    \item A $\Sigma$-algebra $A$ is defined as $A = (S_A,F_A)$ a mathematical
    structure consisting of a carrier set and for each operation name in $ f
    :s_1 ×...× s_n \rightarrow s \in Σ\Sigma$, a function $(f:s_1
    \times...\times s_n \rightarrow s)_A:|A|s_1 \times...\times|A|s_n
    \rightarrow |A|_s$. $|A|_s$ denotes the career set of sort $s$ in $A$.
    Algebra provides an interpretation for the carrier set $S$ and function
    symbols $F$ of a theory.

    \item The type (or language) of the algebra is a set of function symbols. Each
    member of this set is assigned a positive number which is the arity of the
    member. For example, an algebra of type (2,0) denotes an algebra with one binary
    operation and one nullary operation. The group structure defined above is of
    type (2,1,0). That is $∙$ is a binary operation, $^{-1}$ is a unary operation
    and $1$ is the nullary operation.
\end{itemize}
