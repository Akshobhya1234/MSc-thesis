...
\begin{itemize}
    ...
    of single sort. $F$ is the collection of operation names.
    ...
    \item Let $E$ be a set of equations, then model $(Mod_\Sigma(E))$ denotes
    the class of $\Sigma$-algebra satisfying all the equations in $E$.
    \[Mod_\Sigma(E) = \{ A | A \text{ is a } \Sigma-algebra \text{ and A } \models E \} =
    Mod[⟨\Sigma,E⟩]\]

    \item For a $\Sigma$-algebra $A$, the theory of $A$ ($Th(A_\Sigma)$) denotes
    the set of all equations satisfied in $A$. \[Th(A_\Sigma) = \{ e | e \text{is
    an equation and }A \models e \}\]

    \item A presentation is a pair $(\Sigma,E)$ where $\Sigma$ denotes the
    signature and $E$ is a set of equations.
    
    \item A theory can be defined in terms of presentation as a pair $(\Sigma,
    E)$ that is a presentation such that $E$ is closed. A set of equations ($E$)
    is closed if $E = Th_\Sigma(Mod_\Sigma(E))$.
