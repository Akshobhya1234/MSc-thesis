...
example, in Coq, the ring is defined without a multiplicative identity. However,
in Agda, the ring has a multiplicative identity and rng is defined as ringWithoutOne
that has no multiplicative identity. This ambiguity in naming is also seen in
literature. Another example is the same structure having multiple definitions
like Quasigroups. Quasigroups can be defined as a type(2) algebra with Latin
square property or as a type(2,2,2) with left and right division operators. Both
definitions are equivalent, but they are structurally different. This
chapter identifies and classifies five important problems that arise when
defining types of algebraic structures in proof assistant systems. 
...
structures. In this case, rng is used in French whereas ring is in English. These
...
\end{minted} 
\inline{Rng} is defined as \inline{ringWihthoutOne} that is a ring structure without
...
algebraic structure rig is defined as \inline{SemiringWithoutOne} where one
...
set $Q$ and binary operations $∙, \backslash, /$. Quasigroup should satisfy
the below identities that imply left division and right division. 
\[y = x ∙ (x \backslash y)\]
\[y = x \backslash (x ∙ y)\]
\[y = (y / x) ∙ x\]
\[y = (y ∙ x) / x\] 
In the Agda standard library, the quasigroup is defined as a
...
identity. It can be observed the same problem persists in the hierarchy. If
a loop is defined with a quasigroup that is a type (2,2,2) algebra then, a
loop structure of type (2) will be forced to be defined with a suboptimal name.
...
Since quasigroup is defined in terms of division operation, the loop is also defined
as a type (2,2,2) algebra in Agda standard library. The definition of a loop
structure in Agda is as follows:
...
that are commonly used in literature and in other systems. Agda 2.0 removed
