...
must be made on a structurally smaller argument. For the function \inline{_+_}
above, the first argument \inline{n} is smaller in the recursive call
\inline{suc n}. This ensures that the function \inline{_+_} always terminates.
...
equal" is non trivial. \cite{musa} proposes a hierarchy of "sameness".
\begin{enumerate}
...
literally the same symbols. This is called synctatic equality.
  ...
  applying some definitions to synctatic equality.
...
\cite{musa}. This will force to one to use setoid over sets in context of
algebra.
...
\inline{Pointwise} gives the product of equivalence relations in \inline{M} and
\inline{N}. \inline{zip} gives a $\Sigma$-type of dependent pairs.
\inline{isMagma} field is the product of two magma structures.
