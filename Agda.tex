...
\begin{itemize}
  ...
  and \inline{age} are fields of type \inline{String} and \inline{A}
  respectively.
\end{itemize}
...
\inline{Set₂}, and so on. The definition of propositional equality in Agda standard
...
\inline{x}, the elements are related to itself \inline{x ∼ x}.
...
Sym P Q = P ⇒ flip Q
\end{minted}
The first line declares a module \inline{Sym} that takes two arguments:
\inline{P} of type \inline{REL A B ℓ₁} and \inline{Q} of type \inline{REL B A
ℓ₂}. Where \inline{A} and \inline{B} are carrier set over arbitrary universe
level \inline{a} and \inline{b} respectively. The module result type \inline{Set
_}, where the underscore represents a universe level that will be inferred.
\inline{flip} is a function to flip the order of the arguments.
...
\begin{itemize}
  \item Case 1: When \inline{m = n = zero}, Then \inline{zero + zero = zero}
holds by reflexivity. The proof \inline{comm zero zero} represents commutative
...
\item Case 2: \inline{comm zero zero}, in this case, \inline{m} is \inline{zero}
...
The \inline{step-∼} function provide a way to extend an equational proof using
