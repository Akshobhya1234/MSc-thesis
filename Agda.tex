...
\inline{Algebra.Structures} that have an underlying set \inline{A} and the
homogeneous binary relation \inline{_≈_}. The types of algebraic structures are
...
\end{minted}
where \inline{Magma} is the bundled version of the magma structure \inline{M}
and \inline{N} is $M \times N$. \inline{Pointwise} gives the product of
equivalence relations in \inline{M} and \inline{N}. \inline{zip} gives a
$\Sigma$-type of dependent pairs. \inline{isMagma} field is the product of two
magma structures. \inline{×-isEquivalence} says that pointwise preserves the
